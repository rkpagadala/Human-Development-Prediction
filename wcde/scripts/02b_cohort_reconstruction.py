...
Output:
  ...
  wcde/data/processed/cohort_lower_sec_both.csv  (wide: country × cohort_year)
  wcde/data/processed/cohort_primary_both.csv
"""
...
print("Loading raw prop data (this takes ~30s for 172MB file)...")
df = pd.read_csv(os.path.join(RAW, "prop_both.csv"))
print(f"  Rows: {len(df):,}")

df["country"] = df["name"].str.strip()

# Filter to historical obs years and relevant age groups
df = df[df["year"].isin(OBS_YEARS_KEEP)].copy()
df = df[df["age"].isin(AGE_MIDPOINTS.keys())].copy()
df["sex_upper"] = df["sex"].str.title() if "sex" in df.columns else "Both"
df = df[df["sex"].str.lower() == "both"].copy()
...
# For each (country, cohort_year, education_level):
# take the row from the EARLIEST obs_year (= youngest age group = least survivorship bias)
# Rank by obs_year within each cohort group, keep minimum obs_year
df_ranked = df.sort_values(["country","cohort_year","year"])  # sort by year ascending
# Keep the first (earliest) observation of each (country, cohort_year, education) triplet
df_best = df_ranked.groupby(["country","cohort_year","education"], as_index=False).first()

print(f"  Unique (country, cohort_year) pairs: {df_best.groupby(['country','cohort_year']).ngroups:,}")

# Compute completion rates from education distribution
print("Computing completion rates by cohort...")
results = []
for (country, cohort_yr), grp in df_best.groupby(["country","cohort_year"]):
    # Get the obs_year and age_group of the best measurement
    ...
    row = {
        "country": country,
        "cohort_year": cohort_yr,
        "obs_year": best_obs_year,
        "age_group": best_age_group,
    }
    for level, cats in LEVELS_FOR.items():
        vals = [edu_dict.get(c, np.nan) for c in cats]
        valid = [v for v in vals if not np.isnan(v)]
        row[level] = min(sum(valid), 100.0) if valid else np.nan  # cap at 100
    results.append(row)

long = pd.DataFrame(results)
long = long.sort_values(["country","cohort_year"]).reset_index(drop=True)
...
# Spot-check
for c in ["Japan","United States of America","United Kingdom of Great Britain and Northern Ireland",
          "Sri Lanka","Republic of Korea","India"]:
    sub = long[long["country"] == c][["country","cohort_year","primary","lower_sec","upper_sec","college","obs_year","age_group"]]
    sub = sub[sub["cohort_year"].isin([1875,1890,1900,1910,1920,1930,1940,1950,1960,1970,1980,2000,2015])].sort_values("cohort_year")
    if len(sub):
        print(f"\n{c}:")
        print(sub.to_string(index=False))

# Save long format
long_path = os.path.join(PROC, "cohort_completion_both_long.csv")
long.to_csv(long_path, index=False, float_format="%.2f")
print(f"\nSaved: {long_path}")

# Save wide format for lower_sec and primary (most used in generational analysis)
for level in ["lower_sec","primary"]:
    ...
    wide_path = os.path.join(PROC, f"cohort_{level}_both.csv")
...
print("Reliable range for developed countries: cohort years 1890-1960.")
print("Use for: Japan, USA, UK, Germany, France, Australia, New Zealand,")
print("         Canada, Argentina, Chile, Cuba, Uruguay.")
