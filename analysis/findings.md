...
This is the strongest counterargument and we cannot refute it with this data. Colonial and geopolitical factors are not in the dataset. The finding that the tigers were over-performers relative to GDP+parental-education is robust; the causal attribution to "deliberate domestic policy" rather than "inherited institutional capacity from colonial period" is not. Both could be true simultaneously.
...
2. **Education quality.** The dataset measures completion rates, not learning outcomes. A country could achieve 90% secondary completion with low-quality education and see no TFR or health benefit. The findings assume quality is correlated with completion, which may not hold in countries that expanded access rapidly.
